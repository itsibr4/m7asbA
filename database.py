...
class Database:
    ...
    # ---------- المنتجات ----------
    def add_product(self, name, barcode, category, purchase_price, sale_price, quantity, min_quantity):
        cur = self.conn.cursor()
        cur.execute(
            """INSERT INTO products (name, barcode, category, purchase_price, sale_price, quantity, min_quantity)
               VALUES (?,?,?,?,?,?,?)""",
            (name, barcode or None, category, purchase_price, sale_price, quantity, min_quantity),
        )
        self.conn.commit()
        pid = cur.lastrowid
        if quantity:
            # نسجل الحركة في السجل فقط دون خصم/إضافة الكمية مرة أخرى (تم تحديدها بالإدراج أعلاه)
            cur.execute(
