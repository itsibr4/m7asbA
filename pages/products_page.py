...
class ProductsPage(QWidget):
    ...
    def _build_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(
            ["المنتج", "الباركود", "التصنيف", "سعر الشراء", "سعر البيع", "الكمية", "تعديل", "حذف"]
        )
    ...
    def refresh_table_page(self):
        ...
        for row, p in enumerate(page_products):
            ...
            self.table.setItem(row, 3, QTableWidgetItem(app_settings.format_currency(p["purchase_price"])))
            self.table.setItem(row, 4, QTableWidgetItem(app_settings.format_currency(p["sale_price"])))
