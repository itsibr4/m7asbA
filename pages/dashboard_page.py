...
عرض إحصائيات شاملة للمبيعات والمخزون والأرباح
"""
...
class MiniChart(QFrame):
    """رسم بياني مصغر"""
    def __init__(self, title, color="#3b82f6", parent=None):
        super().__init__(parent)
    ...
    def paintEvent(self, event):
        ...
        # Draw bars
        bar_width = (width - 2 * padding) / len(self.data) * 0.6
        gap = (width - 2 * padding) / len(self.data)
        ...
        for i, val in enumerate(self.data):
            ...
            gradient = QLinearGradient(x, y, x, height - padding)
            gradient.setColorAt(0, QColor(self.color))
            gradient.setColorAt(1, QColor(self.color).lighter(150))
...
class DashboardPage(QWidget):
    ...
    def _build_ui(self):
        ...
        self.invoices_card = DashboardCard("🧾", "فواتير اليوم", "0", "عدد الفواتير", "#8b5cf6")
        self.stock_alert_card = DashboardCard("⚠️", "منتجات نافدة", "0", "تحتاج إعادة تعبئة", "#ef4444")

        cards_row1.addWidget(self.sales_card)
        cards_row1.addWidget(self.profit_card)
        cards_row1.addWidget(self.invoices_card)
        cards_row1.addWidget(self.stock_alert_card)
        ...
        self.total_products = DashboardCard("📦", "إجمالي المنتجات", "0", "في قاعدة البيانات", "#f59e0b")
        self.total_customers = DashboardCard("👥", "إجمالي العملاء", "0", "مسجلين في النظام", "#06b6d4")
        self.total_debt = DashboardCard("📋", "الديون المستحقة", "0.00", "على العملاء", "#dc2626")
        self.avg_margin = DashboardCard("📊", "متوسط هامش الربح", "0%", "حسب اليوم", "#ec4899")

        cards_row2.addWidget(self.total_products)
        cards_row2.addWidget(self.total_customers)
        cards_row2.addWidget(self.total_debt)
        cards_row2.addWidget(self.avg_margin)
